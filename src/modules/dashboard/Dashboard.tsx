    ...
    <div className="page-content" style={{ gap: '1.25rem' }}>
      ...
      <div className="dashboard-premium-header">
        <div className="header-content">
          <h1 className="dashboard-title">لوحة التحكم الرئيسية</h1>
          <p className="dashboard-subtitle">مرحباً {currentUser} — نظرة شاملة على حركة اليوم</p>
        </div>
        {activeShift && (
          <div className="active-shift-badge">
            <div className="pulse-dot" />
            <span>صندوق الصراف مفتوح — {activeShift.vaultName}</span>
          </div>
        )}
      </div>

      {/* Premium Alerts Section */}
      {(debtAlerts.length > 0 || assetAlerts.length > 0) && (
        <div className="alerts-container">
          {debtAlerts.length > 0 && (
            <div className="alert-card alert-danger">
              <div className="alert-header">
                <AlertTriangle size={18} />
                <span className="alert-title">تنبيهات الديون ({debtAlerts.length})</span>
              </div>
              <div className="alert-list">
                {debtAlerts.slice(0, 3).map((alert, i) => (
          ...
          {assetAlerts.length > 0 && (
            <div className="alert-card alert-warning">
              <div className="alert-header">
                <AlertTriangle size={18} />
                <span className="alert-title">تنبيهات استحقاق المواعيد والأصول ({assetAlerts.length})</span>
              </div>
              <div className="alert-list">
                {assetAlerts.slice(0, 3).map((alert, i) => (
                  <div key={i} className={`alert-item ${alert.type}`}>
                    <div className="alert-dot" />
                    <span>{alert.text}</span>
                  </div>
      ...
      <div className="quick-actions-section">
        <h2 className="section-subtitle">إجراءات سريعة</h2>
        <div className="quick-actions-grid">
          <button className="quick-action-btn primary" onClick={() => onNavigate('exchange-pos')} title="بيع عملة">
            <TrendingDown size={20} />
            <span>بيع عملة</span>
          </button>
          <button className="quick-action-btn success" onClick={() => onNavigate('exchange-pos')} title="شراء عملة">
            <TrendingUp size={20} />
            <span>شراء عملة</span>
          </button>
          <button className="quick-action-btn secondary" onClick={() => onNavigate('transfers')} title="تحويل من خزنة">
            <ArrowLeftRight size={20} />
            <span>تحويل</span>
          </button>
          <button className="quick-action-btn secondary" onClick={() => onNavigate('customers')} title="إضافة عميل">
            <Building2 size={20} />
            <span>عميل</span>
          </button>
          <button className="quick-action-btn secondary" onClick={() => onNavigate('customer-transactions')} title="تسجيل دين">
            <AlertTriangle size={20} />
            <span>دين</span>
          </button>
          <button className="quick-action-btn secondary" onClick={() => onNavigate('daily-closings')} title="فتح صندوق صراف">
            <Clock size={20} />
            <span>فتح صندوق</span>
          </button>
          <button className="quick-action-btn secondary" onClick={() => onNavigate('daily-closings')} title="إقفال صندوق صراف">
            <XCircle size={20} />
            <span>إقفال صندوق</span>
          </button>
          {isAdmin && (
            <button className="quick-action-btn secondary" onClick={() => onNavigate('currencies')} title="تحديث سعر الصرف">
              <ArrowUpDown size={20} />
              <span>تحديث أسعار</span>
            </button>
          )}
        </div>
      </div>
          ...
          <div className="approvals-list">
            {pendingApprovals.length === 0 ? (
              ...
              <div key={a.id} className="approval-item">
                ...
                <div className="approval-details">{a.details.substring(0, 70)}</div>
                {a.amount && <div className="approval-amount">{a.amount.toLocaleString()} {a.currency}</div>}
      ...
      {/* Currency Balances + Rates Grid */}
      <div className="dashboard-grid-2col">
        <div className="section-card premium-card">
          <div className="section-card-header">
            <div className="section-card-title"><Banknote size={18} color="var(--accent)" />أرصدة العملات</div>
          </div>
          <div className="section-card-body">
            <div className="currencies-grid">
              {currencyBalances.map(c => (
                <div key={c.code} className="currency-balance-card premium">
        ...
        <div className="section-card premium-card">
          ...
          <div style={{ padding: 0 }}>
            <table className="financial-table rates-table">
      ...
      <div className="section-card premium-card">
        ...
        <div style={{ padding: 0 }}>
          <div className="table-responsive">
            <table className="financial-table transactions-table">
              <thead>
